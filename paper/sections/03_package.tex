...
\quad \texttt{model\_artifact\_hash} & str|None & SHA-256 of serialised model \\
\midrule
...
\]
where canonical-JSON is the deterministic serialisation defined by \texttt{json.dumps(..., sort\_keys=True, separators=(",", ":"))}. The exclusion of \texttt{record\_id} from its own hash input is necessary; the exclusion is implemented in \texttt{compute\_record\_id} (\texttt{src/mr\_audit/core/hash.py}).
...
mr-audit inspect  <bundle>          # verify and summarise a bundle
\end{lstlisting}
